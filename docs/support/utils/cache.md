...
封装了storage的相关操作
- sessionStorage（默认）
  - get: cache.session.get(key)
  - set: cache.session.set(key, value)
  - remove: cache.session.remove(key)
  - clear: cache.session.clear()
- localStorage
  - get: cache.local.get(key)
  - set: cache.local.set(key, value)
  - remove: cache.local.remove(key)
  - clear: cache.local.clear()
```javascript
